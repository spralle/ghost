...
describe("resolveRole", () => {
  ...
  test("circular inheritance throws error", async () => {
    ...
    visited.add("user");
    await expect(resolveRole("user", registry, "tenant-1", visited)).rejects.toThrow("Circular inheritance");
  });
  ...
  test("deduplicates same grant from bundle + additionalGrants", async () => {
    ...
    const resolved = await resolveRole(role.id, registry, "tenant-1");
    const viewOrders = resolved.effectiveGrants.filter((g) => g.action === "view" && g.resourceType === "order");
