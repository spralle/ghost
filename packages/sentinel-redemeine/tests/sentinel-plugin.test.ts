...
const mockCheck = mock(
  (): {
    effect: "allow" | "deny";
    matchedRules: { name: string; effect: string; salience: number }[];
    reason: string;
  } => ({ effect: "allow", matchedRules: [], reason: "allowed" }),
);
const mockExpand = mock(
  (): { type: "decision"; description: string; children?: readonly never[]; metadata: Record<string, unknown> } => ({
    type: "decision",
    description: "deny: denied",
    metadata: {},
  }),
);
...
describe("createSentinelPlugin", () => {
  ...
  test("denied command throws AuthorizationError", async () => {
    mockCheck.mockReturnValue({
      effect: "deny",
      matchedRules: [{ name: "r1", effect: "deny", salience: 10 }],
      reason: "denied",
    });
    mockExpand.mockReturnValue({ type: "decision", description: "deny", metadata: {} });
    const plugin = createTestPlugin();
