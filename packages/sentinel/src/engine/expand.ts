...
export function expand(principal: SentinelPrincipal, action: string, context: CheckContext): DerivationNode {
  ...
  const node: DerivationNode =
    ruleChildren.length > 0
      ? {
          type: "decision",
          description: `${result.effect}: ${result.reason}`,
          children: ruleChildren,
          metadata: { effect: result.effect, action },
        }
      : {
          type: "decision",
          description: `${result.effect}: ${result.reason}`,
